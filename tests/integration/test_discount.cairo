...
func test_discounted_buy{syscall_ptr: felt*, range_check_ptr, pedersen_ptr: HashBuiltin*}() {
    ...
    %{ expect_revert(error_message="Invalid discount. Days amount is out of range") %}
    Naming.buy_discounted(naming_contract, token_id2, 282351324389, 189, 0, 456, 1);
    ...
    Naming.buy_discounted(naming_contract, token_id2, 18925, 364, 0, 456, 1);

    // Should fail because the domain is a too long 
    %{ expect_revert(error_message="Invalid discount. Domain length is out of range") %}
    Naming.buy_discounted(naming_contract, token_id2, 130228205358031162777948737262383391520252160213966931801358203517489573947152085, 364, 0, 456, 1);
    ...
    Naming.buy_discounted(naming_contract, token_id2, 282351324389, 364, 0, 456, 2);
