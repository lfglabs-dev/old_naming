...
@external
func test_simple_buy_without_sponsor{syscall_ptr: felt*, range_check_ptr, pedersen_ptr: HashBuiltin*}() {
    alloc_locals;
    ...
    %{
        ...
        stop_mock_1 = mock_call(123, "transferFrom", [1])
        stop_mock_2 = mock_call(10101010, "add_commission", [1])
        warp(1, context.naming_contract)
    %}

    // Set referral contract address
    Naming.set_referral_contract(naming_contract, 10101010);

    let token_id = 1;
    StarknetId.mint(starknet_id_contract, token_id);
    // th0rgal encoded
    let th0rgal_string = 28235132438; // th0rgal encoded

    Naming.buy(naming_contract, token_id, th0rgal_string, 365, 0, 456, 0, new (0));
    let (addr) = Naming.domain_to_address(naming_contract, 1, new (th0rgal_string));
    assert addr = 456;

    Naming.buy(naming_contract, 2, 11111111, 365, 0, 456, 1, new (th0rgal_string));
